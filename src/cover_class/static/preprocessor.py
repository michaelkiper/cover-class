...
from isofit.core.common import resample_spectrum # type: ignore[import]
...
def resample_emit_wls(
    data_matrix: NDArray[np.float32], 
    current_wavelengths: NDArray[np.float32],
    target_wavelengths: NDArray[np.float32],
    fwhm: NDArray[np.float32],
) -> Tuple[FloatTensor, Tensor]:
    s: NDArray[np.float32] = resample_spectrum(data_matrix, current_wavelengths, target_wavelengths, fwhm)
    return FloatTensor(torch.from_numpy(s).to(torch.float32)), torch.from_numpy(target_wavelengths)
